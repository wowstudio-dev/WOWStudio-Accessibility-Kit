	...
	public static function all(): array {
		$rules = array(
			new BrowserRule(
				...
				Severity::Serious,
				Detection::Auto,
				__( 'Text has too little contrast with its background', 'wowstudio-accessibility-remediation' ),
				...
				new FixPlan(
					...
					FixTarget::Css,
					__( 'A colour that clears the required ratio can be computed, and it is written into the site\'s own Additional CSS. The proposal keeps the hue and saturation already chosen and moves only as far as it must.', 'wowstudio-accessibility-remediation' )
				),
				__( 'Anyone with low vision or colour blindness — and anyone reading on a phone in daylight — will struggle to make this out.', 'wowstudio-accessibility-remediation' )
			),
			new BrowserRule(
				...
				Severity::Moderate,
				Detection::Auto,
				__( 'Link inside text is marked only by colour', 'wowstudio-accessibility-remediation' ),
				__( 'This link sits inside a paragraph and is distinguished from the surrounding words by colour alone. Anyone who cannot separate those two colours cannot tell it is a link. Add an underline, or another visible difference that does not depend on colour.', 'wowstudio-accessibility-remediation' ),
				new FixPlan(
					FixKind::Deterministic,
					FixTarget::Css,
					__( 'An underline is what the criterion asks for and what readers already recognise. One rule, no judgement.', 'wowstudio-accessibility-remediation' )
				),
				__( 'Anyone who cannot tell those two colours apart cannot tell there is a link here at all.', 'wowstudio-accessibility-remediation' )
			),
			new BrowserRule(
				'target-too-small',
				'2.5.8',
				Severity::Moderate,
				Detection::Auto,
				__( 'Clickable target is smaller than 24 by 24 pixels', 'wowstudio-accessibility-remediation' ),
				...
				new FixPlan(
					FixKind::Deterministic,
					FixTarget::Css,
					__( 'A minimum size of 24 by 24 is the criterion itself. Applied as a floor, so the control can still grow.', 'wowstudio-accessibility-remediation' )
				),
				__( 'Hard to hit accurately for anyone with a tremor, limited dexterity, or a phone in one hand on a moving bus.', 'wowstudio-accessibility-remediation' )
			),
			new BrowserRule(
				...
				Severity::Serious,
				Detection::Auto,
				__( 'Scrollable area cannot be reached by keyboard', 'wowstudio-accessibility-remediation' ),
				...
				new FixPlan(
					FixKind::Manual,
					FixTarget::Theme,
					__( 'The container needs tabindex and an accessible name. The attribute is mechanical; the name is a description of what is inside, and both live in the theme.', 'wowstudio-accessibility-remediation' )
				),
				__( 'Anyone using a keyboard rather than a mouse can never scroll this, so whatever is out of sight stays out of reach.', 'wowstudio-accessibility-remediation' )
			),
			new BrowserRule(
				...
				Severity::Serious,
				Detection::Auto,
				__( 'Element is hidden from screen readers but still focusable', 'wowstudio-accessibility-remediation' ),
				...
				new FixPlan(
					FixKind::Manual,
					FixTarget::Theme,
					__( 'Either the element should be revealed or it should leave the tab order, and only somebody who knows why it is hidden can say which.', 'wowstudio-accessibility-remediation' )
				),
				__( 'Keyboard users land on something that announces nothing. Focus moves and there is no way to tell where it went.', 'wowstudio-accessibility-remediation' )
			),
		);
